...
#             pose_data['processing_time'] = processing_time
...
#         except Exception as e:
#             logger.error(f"Error in pose detection: {str(e)}")
#             return {'pose_present': False, 'error': str(e)}
...
#             # Draw landmarks
#             if draw_landmarks:
#                 self._draw_landmarks(annotated_frame, landmarks, frame.shape)
        
#         return annotated_frame
    
#     def _draw_landmarks(self, frame: np.ndarray, landmarks: np.ndarray, shape: Tuple):
...
#     def save_learned_patterns(self, poses: List[Dict], output_path: str, 
...
#             poses: List of pose data
#             output_path: Path to save the patterns
#             metadata: Optional metadata to save with patterns
#         """
#         try:
#             # Extract and normalize poses
#             normalized_poses = []
...
#             return {'fps': 0, 'avg_processing_time': 0, 'frame_count': 0}
...
#     def export_pose_data(self, pose_data: Dict, output_path: str, format: str = 'json'):
#         """
#         Export pose data to file
        
#         Args:
#             pose_data: Pose data to export
...
#                 if pose_data['pose_present'] and pose_data['landmarks'] is not None:
#                     import pandas as pd
#                     landmarks = pose_data['landmarks']
                    
#                     # Create DataFrame
#                     df = pd.DataFrame(landmarks, columns=['x', 'y'])
#                     df['landmark_id'] = range(len(landmarks))
#                     df['visibility'] = pose_data['visibility'] if pose_data['visibility'] is not None else [1.0] * len(landmarks)
                    
#                     # Add landmark names
#                     landmark_names = [name for name, idx in sorted(self.pose_landmarks.items(), key=lambda x: x[1])]
#                     df['landmark_name'] = landmark_names[:len(landmarks)]
                    
#                     df.to_csv(output_path, index=False)
                    
#             logger.info(f"Exported pose data to {output_path}")
            
#         except Exception as e:
#             logger.error(f"Error exporting pose data: {str(e)}")
...
# def main():
#     """Example usage of PoseEstimator"""
#     # Initialize the pose estimator
#     estimator = PoseEstimator()
    
#     # Example 1: Process video from webcam
...
#             # Detect pose
#             pose_data = estimator.detect_pose(frame)
            
#             # Draw pose on frame
#             annotated_frame = estimator.draw_pose(frame, pose_data)
            
#             # Get pose angles
#             angles = estimator.get_pose_angles(pose_data)
            
#             # Display pose confidence
#             confidence = estimator.estimate_pose_confidence(pose_data)
...
#                 cv2.putText(annotated_frame, f"{joint}: {angle:.1f}°", 
